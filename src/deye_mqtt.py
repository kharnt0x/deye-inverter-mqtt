...
class DeyeMqttClient:
    def __init__(self, config: DeyeConfig):
        ...
        if config.mqtt.username and config.mqtt.password:
            self._mqtt_client.username_pw_set(username=config.mqtt.username, password=config.mqtt.password)
        self.__status_topic = f"{config.mqtt.topic_prefix}/{config.mqtt.availability_topic}"
        self._mqtt_client.will_set(self.__status_topic, "offline", retain=True, qos=1)
        self.__config = config.mqtt
        self.__mqtt_timeout = 3  # seconds

    def subscribe(self, topic: str, callback):
        self.__log.info("Subscribing to topic: %s", topic)
        result, mid = self._mqtt_client.subscribe(topic, qos=1)
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            while not self._mqtt_client.is_connected():
                time.sleep(1)
            self._mqtt_client.publish(self.__status_topic, "online", retain=True, qos=1)
            self.__log.info(
